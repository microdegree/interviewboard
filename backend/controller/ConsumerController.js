...
router.post("/addNewBooking", (request, response) => {
    ...
    database.collection("interviewBookingTable").insertOne(request.body, (error, result) => {
        if (error) {
...
router.post('/uploadResume', upload.single('resumeUploadFile'), function (request, response) {
...
router.get("/getAllProjects", (request, response) => {
    ...
    database.collection("jobOpeningsTable").find({}).toArray((error, result) => {
        if (error) {
            console.log(error)
            return response.status(500).send(error);
        }
