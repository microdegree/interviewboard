...
router.post("/addProject", (request, response) => {
    ...
    database.collection("jobOpeningsTable").update({ "email": request.body.email },
        request.body,
        { upsert: true },
        (error, result) => {
...
router.post("/getMyProjects", (request, response) => {
    ...
    database.collection("jobOpeningsTable").find(request.body).toArray((error, result) => {
        if (error) {
            console.log(error)
...
router.post("/modifyProject", (request, response) => {
    ...
    database.collection("jobOpeningsTable").replaceOne({ 'email': request.body.email }, request.body, (error, result) => {
        if (error) {
            console.log(error)
...
router.post("/getMyBookings", (request, response) => {
    ...
    console.log('In getMyProjects', request.body)
    database.collection("interviewBookingTable").find(request.body).toArray((error, result) => {
        if (error) {
            console.log(error)
            return response.status(500).send(error);
        }
