...
router.post("/addUser", (request, response) => {
    ...
    database.collection("interviewBoardUserTable").insertOne(request.body, (error, result) => {
        if (error) {
...
router.get("/getAllUsers", (request, response) => {
    ...
    database.collection("interviewBoardUserTable").find({}).toArray((error, result) => {
        if (error) {
            return response.status(500).send(error);
        }
...
router.post("/modifyUser", (request, response) => {
    ...
    database.collection("interviewBoardUserTable").replaceOne({ _id: objectId }, request.body, (error, result) => {
        if (error) {
            console.log(error)
...
router.post("/deleteUser", (request, response) => {
    ...
    var objectId = new ObjectId(request.body._id);
    delete request.body._id
    database.collection("interviewBoardUserTable").deleteOne({ _id: objectId }, (error, result) => {
        if (error) {
            console.log(error)
