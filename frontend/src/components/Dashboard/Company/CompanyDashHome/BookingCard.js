...
const BookingCard = (props) => {
    ...
    return (
        <div>
            <Card style={{ width: '18rem', backgroundColor: '#0000CC', margin: 25 }}>
                <Card.Body>
                    <Card.Title className="bg-primary" style={{ height: '30px', color: '#fff' }}>{userDataWhoBooked.name}</Card.Title>
                    <Card.Subtitle className="mb-2 " style={{ color: '#fff'}} >{props.bookingData.consumerEmail}</Card.Subtitle>
                    <Card.Text style={{ color: '#FF2626', fontWeight: "bold" }}>
                        Applied by above candidate
                    </Card.Text>
                    <Card.Footer  style={{ color: '#fff'}}>  Confirmed Booking</Card.Footer>
                </Card.Body>
            </Card>
        </div>
    )
