...
const SignUp = (props) => {
    ...
    return (
        <div>
            <center>
                <Card style={{ width: '28rem', backgroundColor: '#0000CC' }}>   <br />
                    <Card.Img variant="top" src="https://images.vectorhq.com/images/previews/642/user-icon-psd-452869.png" style={{ width: '5rem', }} />
                    <Card.Body>
                        <Form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <select name="role" ref={register({ required: true })}>
                                <option value="candidateRole">Candidates</option>
    ...
    function updateSignupInfoInServer(requestObject) {
        ...
        fetch('/api/authenticate/signUp', {
            ...
            .then(data => {
                ...
                if ('candidateRole' === requestObject.role) {
                    auth.authenticated = true;
                    console.log('in patient flow signup')
                    props.routeHistory.push("/candidateDashboard");
                } else if ('companyHr' === requestObject.role) {
                    auth.authenticated = true;
                    props.routeHistory.push("/companyDashboard");
